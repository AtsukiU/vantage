  ...
  return (
    <div>
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-2">
          <span
            ...
            style={{ color: committee.agree / committee.total >= 0.6 ? GOOD : "#1c1b18" }}
          >
            {committee.agree}/{committee.total}
          </span>
          <span className="text-[12px] text-[#6c6656]">役が賛成</span>
        </div>
        <button
          type="button"
          onClick={handleDiscuss}
          disabled={loading}
          className="flex items-center gap-1.5 rounded-full bg-[#c9962f] px-3.5 py-1.5 text-[12px] font-bold text-white hover:opacity-90 disabled:opacity-50"
        >
      ...
      <div className="mt-3 grid grid-cols-3 gap-2 sm:grid-cols-6">
        {(Object.keys(committee.roles) as (keyof CommitteeVerdict["roles"])[]).map((role) => {
          const value = committee.roles[role];
          return (
            <div
              key={role}
              className="rounded-lg px-2.5 py-2 text-center"
              style={{ background: value === true ? "rgba(47,158,92,.1)" : "#f0efe6" }}
            >
              <div className="text-[10px] text-[#6c6656]">{ROLE_LABEL[role]}</div>
      ...
      <div className="mt-1.5 flex flex-wrap gap-x-3 gap-y-0.5 text-[10.5px] text-[#a39d8c]">
      ...
      <p className="mt-1.5 text-[10px] text-[#a39d8c]">PM役は保有中のポートフォリオ(セクター・通貨集中度)から判定する参考情報で、上の合議人数(agree/total)には含まれません。保有銘柄がない場合は対象外として賛成扱いになります。</p>

      {error && (
        <p className="mt-3 rounded-lg bg-[#c0392b]/10 px-3 py-2 text-[11.5px] leading-relaxed text-[#c0392b]">{error}</p>
      )}

      {verdict && (
        <div className="mt-4 border-t border-[#e2dfd2] pt-3">
          <div className="rounded-lg bg-[#f7f6f1] p-3">
            <div className="mb-1 text-[11px] font-extrabold text-[#c9962f]">委員会の結論</div>
            <div className="text-[12.5px] leading-relaxed text-[#1c1b18]">{verdict}</div>
          </div>
