  ...
  return (
    <div>
      <div className="flex items-center gap-1.5">
        <TrendingUp size={14} strokeWidth={2.25} className="text-[#c9962f]" />
        <h3 className="text-[13px] font-extrabold text-[#1c1b18]">1ヶ月先の株価傾向(過去データ参考値)</h3>
      </div>
      <p className="mt-1 text-[11px] text-[#6c6656]">
        現在の状況({forecast.momentumLabel} × {forecast.rsLabel})と同じ条件だった過去の事例
        <span className="font-bold text-[#1c1b18]"> {forecast.sampleSize.toLocaleString("ja-JP")}件</span>
      ...
      <div className="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-4">
        <div className="rounded-lg bg-[#f0efe6] px-2.5 py-2 text-center">
          <div className="text-[10px] text-[#6c6656]">上昇していた割合</div>
          <div className="mt-0.5 text-[15px] font-extrabold" style={{ color: forecast.winRatePct >= 50 ? GOOD : BAD }}>
            {forecast.winRatePct.toFixed(1)}%
          </div>
        </div>
        <div className="rounded-lg bg-[#f0efe6] px-2.5 py-2 text-center">
          <div className="text-[10px] text-[#6c6656]">平均リターン</div>
        ...
        <div className="rounded-lg bg-[#f0efe6] px-2.5 py-2 text-center">
          <div className="text-[10px] text-[#6c6656]">中央値</div>
        ...
        <div className="rounded-lg bg-[#f0efe6] px-2.5 py-2 text-center">
          <div className="text-[10px] text-[#6c6656]">上下10-90%レンジ</div>
          <div className="mt-0.5 text-[12px] font-bold text-[#1c1b18]">
            {forecast.p10ReturnPct.toFixed(1)}〜{forecast.p90ReturnPct.toFixed(1)}%
          </div>
        </div>
      </div>

      <p className="mt-3 text-[10.5px] leading-relaxed text-[#a39d8c]">
